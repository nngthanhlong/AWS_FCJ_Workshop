...
- Chọn một khu vực gần đó (ví dụ: us-east-1 hoặc ap-southeast-1).  
- Tạo một IAM role cho Lambda với trust policy:
```json
{
  "Version": "2012-10-17",
  "Statement": [
...
```
- Đính kèm policy `AWSLambdaBasicExecutionRole` cho role.  
- Đặt CloudWatch Logs retention (ví dụ: 7–14 ngày) để kiểm soát chi phí: CloudWatch → Log groups → `/aws/lambda/<function-name>` → Actions → Edit retention.
